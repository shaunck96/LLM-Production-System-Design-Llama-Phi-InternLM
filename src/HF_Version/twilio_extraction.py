...
class SafeQueryDSL:
    def __init__(self, spark):
        """
        Initializes a SafeQueryDSL instance.

        Args:
            spark (SparkSession): The SparkSession to use for building queries.
        """
        self.spark = spark
        self.query_parts = []
    ...
    def select(self, *columns):
        ...
        Args:
            *columns (str): The columns to include in the SELECT clause.

        Returns:
            SafeQueryDSL: The instance itself to allow method chaining.
        """
    ...
    def from_table(self, table):
        ...
        Args:
            table (str): The table to include in the FROM clause.

        Returns:
            SafeQueryDSL: The instance itself to allow method chaining.
        """
        self.query_parts.append(("FROM", table))
        return self
    ...
    def join(self, table, condition):
        ...
        Args:
            ...
            condition (str): The condition for the JOIN clause.
    ...
    def where(self, condition):
        ...
        Returns:
            SafeQueryDSL: The instance itself to allow method chaining.
        """
        self.query_parts.append(("WHERE", condition))
        return self

    def build(self):
        """
        Builds the final SQL query from the added query parts.
        ...
        for part_type, part_value in self.query_parts:
            ...
            elif part_type == "WHERE":
                query += f"WHERE {part_value} "
        return query
...
class TwilioCallExtractor:
    ...
    def setup_dates(self) -> None:
        ...
        self.start_date = (pd.to_datetime(self.end_date) - timedelta(days=365)).strftime("%Y-%m-%d")
    ...
    def customer_metadata_extraction(self) -> pd.DataFrame:
        ...
        try:
            YEAR, MONTH = str(self.start_date).split('-')[0],self.start_date.split('-')[1]
    ...
    def recording_download_trigger(self) -> pd.DataFrame:
        ...
        3. Builds and executes a SQL query to retrieve task segments.
        ...
        try:
            ...
            query = (self.dsl
                .select("t.*", "extract_recording_sid(t.taskURL) as RecordingSID")
                .from_table("csda.tasksegments t")
                .join("task_ids_temp tid", "t.taskId = tid.taskId")
                .where("DATE(t.agentConnectTimestamp) >= ?")
                .where("t.taskId IN (SELECT taskId FROM task_ids_temp)")
                .build())

            dfFinal = spark.sql(query, params=[self.start_date])

            # Then, when executing the query:
            dfFinal = spark.sql(query, (self.start_date,))

            extract_recording_sid_udf = udf(self.extract_recording_sid, StringType())
            dfFinal = dfFinal.withColumn("RecordingSID", extract_recording_sid_udf(col("taskURL")))

            dfFinal = dfFinal.toPandas()
            dfFinal = dfFinal[dfFinal['RecordingSID'] != '']

            print("Number of Records to be processed: "+str(len(dfFinal)))
